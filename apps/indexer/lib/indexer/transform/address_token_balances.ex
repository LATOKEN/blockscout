...
defmodule Indexer.Transform.AddressTokenBalances do
  ...
  defp reducer({:token_transfers_params, token_transfers_params}, initial) when is_list(token_transfers_params) do
    ...
    |> ignore_burn_address_transfers_for_token_larc_721
     |> Enum.reduce(initial, fn %{
      ...
      if params[:token_ids] && token_type == "LRC-1155" do
        params[:token_ids]
        |> Enum.reduce(acc, fn _, sub_acc ->
          sub_acc
          |> add_token_balance_address(from_address_hash, token_contract_address_hash, block_number, token_type)
          |> add_token_balance_address(from_address_hash, token_contract_address_hash, block_number, token_type)
        end)
      else
        acc
  ...
  defp add_token_balance_address(map_set, unquote(@burn_address), _, _, _), do: map_set

  defp add_token_balance_address(map_set, address, token_contract_address, block_number, token_type) do
    MapSet.put(map_set, %{
