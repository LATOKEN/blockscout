defmodule Indexer.TokenBalances do
  ...
  * `token_id` - token id for LARC-1155 tokens
  """
  ...
  def fetch_token_balances_from_blockchain(token_balances) do
    ...
    regular_token_balances =
      ...
      |> Enum.filter(fn request ->
        if Map.has_key?(request, :token_type) do
          request.token_type !== "LARC-1155"
        else
          true
        end
      end)

    larc1155_token_balances =
      token_balances
      |> Enum.filter(fn request ->
        if Map.has_key?(request, :token_type) do
          request.token_type == "LARC-1155"
    ...
    requested_larc1155_token_balances =
      larc1155_token_balances
      |> BalanceReader.get_balances_of_with_abi(@larc1155_balance_function_abi)
      |> Stream.zip(larc1155_token_balances)
      |> Enum.map(fn {result, token_balance} -> set_token_balance_value(result, token_balance) end)

    requested_token_balances = requested_regular_token_balances ++ requested_larc1155_token_balances
