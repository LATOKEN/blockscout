defmodule BlockScoutWeb.Tokens.Helpers do
  ...
  When the token's type is LARC-20, then we are going to format the amount according to the token's
  decimals considering 0 when the decimals is nil. Case the amount is nil, this function will
  return the symbol `--`.

  When the token's type is LARC-721, the function will return a string with the token_id that
  represents the LARC-721 token since this kind of token doesn't have amount and decimals.
  """
  def token_transfer_amount(%{token: token, amount: amount, token_id: token_id}) do
    do_token_transfer_amount(token, amount, token_id)
  end

  defp do_token_transfer_amount(%Token{type: "LARC-20"}, nil, _token_id) do
    {:ok, "--"}
  end

  defp do_token_transfer_amount(%Token{type: "LARC-20", decimals: nil}, amount, _token_id) do
    {:ok, CurrencyHelpers.format_according_to_decimals(amount, Decimal.new(0))}
  end

  defp do_token_transfer_amount(%Token{type: "LARC-20", decimals: decimals}, amount, _token_id) do
    {:ok, CurrencyHelpers.format_according_to_decimals(amount, decimals)}
  end

  defp do_token_transfer_amount(%Token{type: "LARC-721"}, _amount, _token_id) do
    {:ok, :larc721_instance}
  end
