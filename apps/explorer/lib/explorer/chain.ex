  ...
  *NOTE*
  ...
  def stream_unfetched_token_instances(initial, reducer) when is_function(reducer, 2) do
    nft_tokens =
      from(
        token in Token,
        where: token.type == ^"LARC-721" or token.type == ^"LARC-1155",
        select: token.contract_address_hash
      )
  ...
  @spec transaction_token_transfer_type(Transaction.t()) ::
          :larc20 | :larc721 | :larc1155 | :token_transfer | nil
  ...
  defp find_token_transfer_type(transaction, input, value) do
    ...
    case {to_string(input), value} do
      ...
      {"0xf242432a" <> params, ^zero_wei} ->
        ...
        find_larc1155_token_transfer(transaction.token_transfers, {from_address, to_address})
      ...
      # check for LARC-20 or for old LARC-721, LARC-1155 token versions
      {unquote(TokenTransfer.transfer_function_signature()) <> params, ^zero_wei} ->
        ...
        find_larc721_or_larc20_or_larc1155_token_transfer(transaction.token_transfers, {address, decimal_value})
  ...
  defp find_larc1155_token_transfer(token_transfers, {from_address, to_address}) do
    token_transfer =
      Enum.find(token_transfers, fn token_transfer ->
        token_transfer.from_address_hash.bytes == from_address && token_transfer.to_address_hash.bytes == to_address
      end)

    if token_transfer, do: :larc1155
  end

  defp find_larc721_or_larc20_or_larc1155_token_transfer(token_transfers, {address, decimal_value}) do
    token_transfer =
      Enum.find(token_transfers, fn token_transfer ->
